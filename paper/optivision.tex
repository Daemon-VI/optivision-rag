...
\label{sec:sweep}
Under E1 the token-budget sweep (Fig.~\ref{fig:sweep}) is almost flat. Keeping the
...
\caption{\textbf{E1.} The token-budget sweep, all rows using one-bit codes.
Varying the retained fraction from $10\%$ to $50\%$ moves nDCG@5 retention by
$0.6$ points, while the gap to the unquantized pruned index (dashed) is over $10$
points. Under E2 the same sweep spans $25.8$ points on \texttt{docvqa}.}
\label{fig:sweep}
\end{figure}
