...
encoder, and it flips only queries whose float-precision margin over the
best competitor is smaller than that noise --- under $1\%$ of $945$ real ViDoRe
queries above twice the noise, against a third of those below half. The generated corpus decides $60$ of its $72$ queries by three digits at
...
analysis over eight encode caches and two dense ViDoRe splits locates the
one-bit codec's cost in the queries' decision margins rather than in the
encoder: the codec's score noise is the same under both encoders, and it flips
only queries decided by less than that noise --- under $1\%$ of $945$ real
ViDoRe queries above twice the noise, against a third of those below half. The
same matched-budget study on those splits shows that
selecting tokens by embedding coverage beats pixel saliency, and that uniformly
random probes do as well as a designed pixel-saliency selector --- the control
any future selector has to clear.
\end{enumerate}
...
Binary quantization's cost also has a documented cause in the single-vector
literature: it depends on the encoder's training objective through the
Gaussianity of its coordinates, giving a pairwise misordering bound
$P \le \exp(-\gamma^2 b^2 \Delta^2 / 2\sigma^2)$ in the score gap $\Delta$ and
noise variance $\sigma^2$~\cite{covariance2026} --- a tail bound, not a
per-query test. The same margin-vs-noise mechanism explains quantized
classifier and LLM-agent decisions~\cite{hu2022quantized,lowbitdecisions2026},
but neither line applies it per query through a MaxSim maximum, or validates a
flip threshold on a retrieval benchmark.

\subsection{Compressing multi-vector visual indexes}
Token pooling~\cite{clavie2024pooling} evaluates pooling alone and pooling
with 2-bit PLAID compression as two separate pipelines, never decomposed
against a shared baseline. CRISP~\cite{crisp2025} folds clustering into
training on a single encoder with no quantization axis.
HPC-ColPali~\cite{hpccolpali2025} defines a pruning-ablated baseline for
exactly this purpose, but states that ``all numerical results presented
herein are estimated based on the theoretical advantages of HPC-ColPali's
design and typical performance gains observed in similar research'' --- the
ablation is designed, not measured. DocPruner~\cite{docpruner2025}, Structural
Anchor Pruning~\cite{structuralanchor2026} and a Voronoi-cell
formulation~\cite{voronoi2026} refine which tokens to prune, without adding a
codec. SaMer~\cite{samer2026} instead shows random and spatial pruning sit
within 0.2 Flickr30K R@1 points of each other while merging adds four to five
points, ``because pruning removes visual evidence before the query is known,
whereas merging keeps it in aggregated representatives'';
MarginMerge~\cite{marginmerge2026} and Sculpting~\cite{sculpting2026} report
the same pattern (Sculpting: nDCG@5 0.86 vs. DocPruner's 0.77 at comparable
compression). None of these seven systems isolate pruning from quantization
on shared cached vectors with exact MaxSim and rank correlation.

\subsection{Positioning}
The individual components we use are not novel, and we do not claim them.
What is missing is a controlled attribution of quality loss \emph{between}
pruning and quantization on identical cached vectors under exact MaxSim ---
a decomposition that
\cite{clavie2024pooling,crisp2025,hpccolpali2025} each stop short of. Systems
report the product of the two savings; we report the ratio of the two costs.
Our random-probe control (Contribution 4) applies to token \emph{selection}
within the pruning family, where SaMer's own numbers show designed and random
pruning tie; merging~\cite{samer2026,marginmerge2026,sculpting2026} is the
direction that escapes that tie, and we do not claim to beat it. Whether the
pruning-versus-codec attribution itself reverses across encoders or corpora,
which none of the above measure, is the question this paper answers.
...
from the ones it loses with AUC $0.78$--$0.95$ on the generated caches. The same
rule now holds at far higher power on $945$ real ViDoRe queries: of $698$ above
$2\sigma$, $3$ flip ($0.4\%$), against $32$--$38\%$ of those below $0.5\sigma$,
with AUC $0.92$--$0.96$ (Table~\ref{tab:margin}).

\begin{table}[!t]
\caption{Per-query decision margin against codec noise, both as a fraction of
the gold page's float score. $m$ is the median float margin: over the $60$
precise queries for the generated rows, and over all $494$ / $451$ queries of
the \texttt{infovqa} / \texttt{docvqa} ViDoRe splits for the E2 rows; $\sigma$
is the score noise a codec adds; flip rates are the share of queries whose
top-$1$ result changes under the sign code, by $|m|/\sigma$ bin; AUC is how
well $m$ alone separates the float winners the sign code keeps from those it
loses. The first five rows are the E1 encoder (ColSmol) and the E3 encoder
(ColPali) on the same $60$ pages with the code's glyphs at the stated size;
every row re-renders only those glyphs, leaving every other pixel fixed.
The last two rows are ColPali-v1.3 on the real \texttt{infovqa} and
\texttt{docvqa} test splits (E2).}
\label{tab:margin}
\centering
\footnotesize
...
ColPali $3\times$ & $-1.41\%$ & $2.3\%$ & $1.4\%$ & $33\%$ & $0\%$ & $0.83$ \\
\midrule
ColPali, infovqa (E2) & $+12.49\%$ & $2.7\%$ & $1.5\%$ & $38\%$ & $0.8\%$ & $0.96$ \\
ColPali, docvqa (E2) & $-0.07\%$ & $2.2\%$ & $1.3\%$ & $32\%$ & $0\%$ & $0.92$ \\
\bottomrule
\end{tabular}
\end{table}
...
code's glyphs three times larger, with every other pixel, the generator's
random sequence and the queries held fixed, does not get ColPali off the
floor: it wins $16$ of the $60$ precise queries in float (median precise
margin $-1.41\%$, against $-1.39\%$ at $1\times$), and the one-bit cost goes
negative, from $+1.6\%$ to $-1.6\%$ (R@1 $0.389 \to 0.417$). The sign code
changes $12$ top-$1$ results, $7$ lost-to-won against $5$ won-to-lost, netting
the precise count from $16$ to $18$ of $60$ --- the same reshuffle-at-the-floor
pattern as $1\times$. An earlier render that enlarged the code's glyphs inside
the text flow, which also shifted every line below them, had given a different
picture --- precise R@1 lifting to $0.317$ and the one-bit cost rising to
$4.1\%$, with precise R@1 under the sign code falling to $0.233$ --- but that
was the layout change, not the glyph size: with only the glyphs enlarged, the
effect vanishes. At $72$ queries every aggregate delta is inside the paired
confidence interval, so we report the per-query structure in
Table~\ref{tab:margin} --- $0$ of $18$ queries above $2\sigma$ flip, $33\%$ of
those below $0.5\sigma$ do, AUC $0.83$ --- as the measurement. The same
manipulation on ColSmol, at
...
to begin with. The same rule holds, at far higher statistical power, on the two
dense ViDoRe splits: of $698$ real queries above $2\sigma$, $3$ flip ($0.4\%$),
against $32$--$38\%$ of those below $0.5\sigma$. On the two splits a sign code
flips $21$ top-$1$ results on \texttt{infovqa} and $23$ on \texttt{docvqa} ---
$5.5\%$ and $8.2\%$ of queries sit within about $3\%$ of a tie, respectively;
the splits' median float margins are $+12.5\%$ and $-0.07\%$, i.e.\
\texttt{docvqa}'s float retriever is already losing half its queries outright
before any codec is applied --- while nDCG@5 reports $97.4\%$ and $96.3\%$
retention, a reason to report top-$1$ agreement alongside $\tau$.
...
operative variable. The decision-margin rule of Finding~3, verified on the
generated corpus across eight caches, is now also measured on the two dense
ViDoRe splits ($945$ real queries, ColPali-v1.3): $3$ of $698$ queries above
$2\sigma$ flip ($0.4\%$), against $32$--$38\%$ below $0.5\sigma$.
...
$2$--$3\%$ of score noise under either encoder, and it costs only the queries
whose float margin over the best competitor is smaller than that noise ---
under $1\%$ of $945$ real ViDoRe queries above twice the noise, against a third
of those below half. The generated corpus decides its precise
...
Future work follows directly: place the two-bit code, which halves
...
\bibitem{crisp2025}
J. Veneroso, R. Jayaram, J. Rao, G. Hern\'andez \'Abrego, M. Hadian and D. Cer,
``CRISP: Clustering Multi-Vector Representations for Denoising and Pruning,''
\emph{arXiv preprint arXiv:2505.11471}, 2025.

\bibitem{hpccolpali2025}
D. Bach, ``Hierarchical Patch Compression for ColPali: Efficient Multi-Vector
Document Retrieval with Dynamic Pruning and Quantization,'' \emph{arXiv
preprint arXiv:2506.21601}, 2025.
...
\emph{arXiv preprint arXiv:2601.20107}, 2026.

\bibitem{marginmerge2026}
A. Mahdizadeh, A. Salari, S. Rajabi, S. Mirabbasi, P. Nasiopoulos and
A. Morsali, ``Coverage Matters: MarginMerge for Compressing Multi-Vector
Visual Document Retrievers,'' \emph{arXiv preprint arXiv:2608.02969}, 2026.

\bibitem{voronoi2026}
Y. Kankanampati, Y. Zong, N. Tomeh, B. Piwowarski and J. Le Roux, ``A Voronoi
Cell Formulation for Principled Token Pruning in Late-Interaction Retrieval
Models,'' in \emph{Proc. 49th Int. ACM SIGIR Conf. on Research and Development
in Information Retrieval}, 2026. arXiv:2603.09933.

\bibitem{samer2026}
S. Park, J. Jung, J. Park and J. Kang, ``Do All Visual Tokens Matter Equally?
Object-Evidence Preserving Token Merging for Vision-Language Retrieval,''
\emph{arXiv preprint arXiv:2607.04605}, 2026.

\bibitem{covariance2026}
W. Xiao, ``Covariance Structure and Coordinate Heterogeneity Govern Binary
Quantization of Contrastive Embeddings,'' \emph{arXiv preprint
arXiv:2605.17524}, 2026.

\bibitem{lowbitdecisions2026}
Z. Wu, S. Dhiman and A. Koshiyama, ``Which Decisions Low-Bit Quantization
Breaks, and How to Predict Them,'' \emph{arXiv preprint arXiv:2608.06564},
2026.

\bibitem{hu2022quantized}
Q. Hu, Y. Guo, M. Cordy, X. Xie, W. Ma, M. Papadakis and Y. Le Traon,
``Characterizing and Understanding the Behavior of Quantized Models for
Reliable Deployment,'' \emph{arXiv preprint arXiv:2204.04220}, 2022.
